...
export const availableModules = {
  ...
  projects: async () =>
    (await import("@/modules/projects/projects.routes")).default,
...
type DeploymentTargetsShape = Record<
  string,
  { port: number; modules: ModulesTuple }
>;
...
export function resolveDeploymentTarget(targetName?: string): {
  ...
  const resolvedName =
    targetName && isDeploymentTargetName(targetName)
      ? targetName
      : fallbackName;
